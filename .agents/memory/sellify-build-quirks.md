...
## EAS env can leak pk_test Clerk keys into release builds
Symptom: TestFlight sign-in works but every API call 401s; server logs show authHeader true, iss = *.clerk.accounts.dev, token not expired. Cause: a pk_test EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY in the builder's EAS env gets baked into the release binary, so the app authenticates against the dev Clerk instance while the prod API verifies against the live one.
**Rule:** release builds (__DEV__ false) must ignore pk_test keys and derive the pk_live key from EXPO_PUBLIC_DOMAIN (see lib/clerkConfig.ts guard). **How to detect:** server 401 logging with the token's iss claim — keep that pattern for any auth bug.
